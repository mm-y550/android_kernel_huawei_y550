	...
	if (rc){
		...
		if(!use_other_charger){
			if(!dsm_client_ocuppy(charger_dclient)){
				...
				dsm_client_notify(charger_dclient, DSM_CHARGER_SPMI_ABNORMAL_ERROR_NO);
			}
		}
#endif
	}
...
int hw_discharge_voltage_check(struct qpnp_bms_chip *chip,int soc,int vbat_uv,int batt_temp)
{
	...
	/* as prev_soc_uuc is not accurate lead to soc_ocv is higher than the real ocv
		limit soc_ocv lower than soc +3 */
	...
	pr_debug("low_voltage_flag=%d,last_ocv_uv=%d,soc=%d,vbat_index=%d,vbat_sample_num=%d,soc_ocv =%d\n",low_voltage_flag,chip->last_ocv_uv,soc,chip->vbat_index,chip->vbat_sample_num,soc_ocv);
	return soc;
}
...
static void monitor_soc_work(struct work_struct *work)
{
	...
	if (!is_battery_present(chip)) {
		...
		if(!use_other_charger){
			...
			if((OCP_CURRENT <= chip->current_now) || (HIGH_VOL <= vbat_uv)
				||(LOW_VOL >=vbat_uv) ||(HOT_TEMP <= batt_temp) ||(LOW_TEMP >= batt_temp)){
					pr_err("Find abnormal ADC values! The values are:"
							"chip->current_now=%d vbat_uv=%d batt_temp=%d\n",
							chip->current_now,
							vbat_uv,
							batt_temp);
			...
			if(shutdown_soc_valid && (SOC_INVALID != chip->shutdown_soc)){
				pr_info("shutdown_soc =%d, power on new_soc=%d\n",
						chip->shutdown_soc, new_soc);
				if(abs(chip->shutdown_soc - new_soc) > SOC_POWERON_DELTA){
					pr_err("soc changed more than 10 percent between this power"
							"on new soc and last shutdown soc"
							"chip->shutdown_soc=%d new_soc=%d\n",
							chip->shutdown_soc,
							new_soc);
					dump_registers_and_adc(bms_dclient, g_lbc_chip, DSM_BMS_POWON_SOC_ERROR_NO);
				}
