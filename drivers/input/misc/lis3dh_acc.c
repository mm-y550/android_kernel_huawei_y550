				...
				u8 *buf, int len)
{
	int err;
	int tries = 0;
	struct lis_gs_dsm_operation *pt_lis3dh_dsm_ops = &acc->lis3dh_dsm_operation;
	...
	if (err != 2) {
		...
		acc->lis_gs_exception.i2c_err_num = err;
		pt_lis3dh_dsm_ops->dump_i2c_status(acc);
	} else {
		err = 0;
	}
...
#ifdef CONFIG_HUAWEI_KERNEL
	struct lis_gs_dsm_operation *pt_lis3dh_dsm_ops = &acc->lis3dh_dsm_operation;
	pt_lis3dh_dsm_ops->judge_check_excep(xyz[0],xyz[1],xyz[2],acc);

	/* store the current x,y,z value for forc dump to read current value*/
	memcpy(acc->lis_gs_exception.xyz, xyz,3*sizeof(int));
#endif
