...
struct lis_gs_dsm_operation{
	ssize_t (*report_dsm_err)(int type,bool auto_report_flag);
	int (*dump_i2c_status)(struct lis3dh_acc_data *acceld);
	int (*judge_check_excep)(int x, int y, int z, struct lis3dh_acc_data *acceld );
};
